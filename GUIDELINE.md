...
4. Use `python .codex-swarm/agentctl.py task list` / `python .codex-swarm/agentctl.py task show T-123` to inspect tasks, and run `python .codex-swarm/agentctl.py task lint` after changes to keep `tasks.json` checksum-valid.
...
2. Count `tasks.json` as the single source of truth for every task.
...
3. **Workflow mode.** `.codex-swarm/swarm.config.json` controls `workflow_mode`:
   - `direct`: low-ceremony, single-checkout workflow (task branches/worktrees and PR artifacts are optional).
   - `branch_pr`: strict branching workflow (per-task branches/worktrees + tracked PR artifacts; `tasks.json` is single-writer and updated on `main` only).
...
- `python .codex-swarm/agentctl.py task lint`: Validate schema/deps/checksum for `tasks.json`.
...
5. **Finish + cleanup (branch_pr).** INTEGRATOR (on clean `main`) runs `python .codex-swarm/agentctl.py pr check T-123`, then `python .codex-swarm/agentctl.py integrate T-123 --branch task/T-123/<slug> --run-verify`, and commits the resulting `tasks.json` change (plus docs updates) as the closure commit.

## 9. Troubleshooting & best practices

- `python .codex-swarm/agentctl.py task lint` fails with a checksum error: `tasks.json` was likely edited by hand. Undo the manual edit (e.g., `git checkout -- tasks.json`) and reapply the change via `python .codex-swarm/agentctl.py task update/comment/set-status` so the checksum stays valid.
