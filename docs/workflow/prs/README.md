...
## Handoff notes → tasks.json

Executors (CODER/TESTER/DOCS/REVIEWER) should leave short handoff notes in `docs/workflow/prs/T-123/review.md` under `## Handoff Notes` (e.g., `- CODER: ...`).

In `workflow_mode=branch_pr`, INTEGRATOR appends these notes into `tasks.json` when closing the task via `finish` (as part of `integrate`).
