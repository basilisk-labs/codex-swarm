...
- Add `task scaffold` (create `docs/workflow/T-###.md` from a template).
...
- Implemented new subcommands in `scripts/agentctl.py`:
  ...
  - `task scaffold` writes `docs/workflow/T-###.md` skeletons (uses existing task title by default).
...
- Updated `docs/agentctl.md` to document the new helpers.
