...
- Make `agentctl verify` auto-append to `docs/workflow/T-###/pr/verify.log` when a PR artifact exists, so callers don’t need to pass `--log` every time.
...
- Manual smoke:
  ...
  - `python scripts/agentctl.py verify T-076` (should append to `docs/workflow/T-076/pr/verify.log`)
