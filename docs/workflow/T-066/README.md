...
- `agentctl`:
  ...
  - Add `pr` commands to create/update/check tracked PR artifacts under `docs/workflow/T-###/pr/`.
...
- Repo structure + docs:
  - Track PR artifacts under `docs/workflow/T-###/pr/` and ignore `.codex-swarm/worktrees/` (legacy `docs/workflow/prs/` is deprecated).
...
- Added `agentctl` commands:
  ...
  - `pr open/update/check` for tracked PR artifacts under `docs/workflow/T-###/pr/`.
