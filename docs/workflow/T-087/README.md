...
- When starting Codex Swarm in an existing git repo, automatically pin the current branch as the base branch (stored in local git config as `codexswarm.baseBranch`) and use it as the default base for creating task branches/worktrees and for integration.
...
- `scripts/agentctl.py`: replace hard-coded `main` defaults/guardrails with “base branch” derived from pinned git config (auto-pinned on first use).
...
- Manual smoke:
  ...
  - Run `git config --unset codexswarm.baseBranch` (if it is set).
  - Run `python scripts/agentctl.py work start T-087 --agent CODER --slug smoke --worktree` and confirm `git config --get codexswarm.baseBranch` is now `tmp/base-smoke`.
