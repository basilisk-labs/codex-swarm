...
- `scripts/agentctl.py`: update `integrate` to avoid warning spam when PR meta `head_sha` is stale; write correct `head_sha` and `last_verified_sha` into `.codex-swarm/workspace/T-073/pr/meta.json` on `main` after merge.
...
- `scripts/agentctl.py`:
  ...
  - After merge, ensure `.codex-swarm/workspace/T-###/pr/meta.json` on `main` reflects the correct `head_sha` and `last_verified_sha` for the integrated branch head.
