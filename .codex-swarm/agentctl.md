...
# comment-driven commits (no direct git): agentctl builds `<emoji> <suffix> <comment>` from your status body
...
- Do not run git directly; use the comment-driven flags (`--commit-from-comment`, `--status-commit`) so agentctl handles staging/commits for you.
